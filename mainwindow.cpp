    ...
    , ui(new Ui::MainWindow)
{
	...
	// diyCCB的特殊控制，不让其导致窗口过度拉伸
	ui->diyCCB->setSizeAdjustPolicy(QComboBox::AdjustToMinimumContentsLengthWithIcon);
	ui->diyCCB->view()->setTextElideMode(Qt::ElideRight);
}
...
void MainWindow::on_diyDelButton_clicked()
{
	int index = ui->diyCCB->currentIndex();
	if (index == -1)
		tip(ui->mainShowWidget, "不存在可删除项");
	else
		ui->diyCCB->removeItem(ui->diyCCB->currentIndex());
}
