...
void CCBIni(map<QString,QString>&read,QString name,QComboBox* ccb)
{
	...
	QString defaultIndexName = "default" + name;
	DBread[defaultIndexName] = "";
	...
	read[defaultIndexName] = DBread[defaultIndexName];
...
void ini(Ui::MainWindow* ui)
{
    ...
    QString chatgpt=read["chatgpt"];
    if(chatgpt.size())
        ui->chatgptText->setText(chatgpt);
	...
	QString zlibrary=read["zlibrary"];
    if(zlibrary.size())
        ui->zlibraryText->setText(zlibrary);
...
void save(Ui::MainWindow* ui)
{
    ...
    data.push_back(pair{"chatgpt",qs});

    qs = ui->deepseekText->displayText();
	...
	qs = ui->zlibraryText->displayText();
	data.push_back(pair{"zlibrary", qs});
